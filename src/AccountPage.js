...
<button onClick={onBack} style={{ marginTop: 16, background: "none", border: "none", color: "#4a6a4a", cursor: "pointer", fontSize: 14, fontFamily: "'Georgia', serif" }}>
  ...
  return (
<div style={{ height: "100dvh", background: "#060e06", fontFamily: "'Georgia', serif", color: "#e8f0e8", display: "flex", flexDirection: "column", overflowY: "auto" }}>
      {/* Header */}
<div style={{ padding: "16px 20px", borderBottom: "1px solid #1a2e1a", display: "flex", alignItems: "center", gap: 12, background: "#060e06", position: "sticky", top: 0, zIndex: 10 }}>
...
<div style={{ fontSize: 12, color: "#4a6a4a" }}>{user.email}</div>
      ...
      {/* Tabs */}
<div style={{ display: "flex", borderBottom: "1px solid #1a2e1a", background: "#060e06", position: "sticky", top: 61, zIndex: 10 }}>
        {[
...
<Bell size={11} />
                      Daily email when tee times available
</div>
</div>
<button onClick={() => removeAlert(alert.id)}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#e87070", padding: 8 }}>
<Trash2 size={18} />
</button>
</div>
              ))
            ...
            {alerts.length > 0 && (
<div style={{ fontSize: 12, color: "#3a5a3a", textAlign: "center", marginTop: 16 }}>
...
<EmptyState
              icon="⛳"
              message="No upcoming tee times."
              sub="Book a tee time through GolfNow and it will appear here once we connect the live API."
            />
<div style={{ marginTop: 24, background: "#0a160a", border: "1px solid #1a2e1a", borderRadius: 14, padding: 16, textAlign: "center" }}>
<div style={{ fontSize: 14, color: "#5a7a5a", marginBottom: 12 }}>
                Book tee times at any of your saved courses
</div>
...
const LoadingState = () => (
<div style={{ textAlign: "center", padding: "40px 20px", color: "#3a5a3a", fontSize: 14 }}>
    Loading...
</div>
);
