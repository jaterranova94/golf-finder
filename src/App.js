...
export default function App() {
  ...
  const filteredCourses = useMemo(() => {
    return BOSTON_COURSES.filter((course) => {
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (filters.maxPrice !== 999) count++;
    if (filters.timeRange !== "all") count++;
    if (filters.maxDistance !== 999) count++;
    if (filters.walkableOnly) count++;
    return count;
  }, [filters]);
  const handleMarkerClick = useCallback((course) => {
...
<div style={{ position: "absolute", top: 76, left: "50%", transform: "translateX(-50%)", zIndex: 100, background: "rgba(10,20,10,0.92)", border: "1px solid #1e3a1e", borderRadius: 30, display: "flex", padding: 3 }}>
        {[{ id: "map", icon: <MapIcon size={14} />, label: "Map" }, { id: "list", icon: <List size={14} />, label: "List" }, { id: "favorites", icon: <Heart size={14} />, label: `Saved ${favorites.length > 0 ? `(${favorites.length})` : ""}` }].map((tab) => (
<button key={tab.id} onClick={() => { setView(tab.id); setSelectedCourse(null); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 14px", borderRadius: 28, border: "none", background: view === tab.id ? "#1a3a1a" : "transparent", color: view === tab.id ? "#b8d8b8" : "#4a6a4a", cursor: "pointer", fontSize: 13, fontFamily: "'Georgia', serif" }}>
            {tab.icon}{tab.label}
</button>
        ))}
</div>
...
<NavigationControl position="bottom-right" style={{ marginBottom: 100 }} />
          {filteredCourses.map((course) => {
            ...
            return (
<Marker key={course.id} latitude={course.lat} longitude={course.lng} anchor="bottom" onClick={(e) => { e.originalEvent.stopPropagation(); handleMarkerClick(course); }}>
...
<div style={{ position: "absolute", inset: 0, overflowY: "auto", paddingTop: 120, paddingBottom: 20, background: "#060e06" }}>
          {filteredCourses.length === 0 ? <EmptyState message="No courses match your filters." /> : filteredCourses.map((course) => <CourseListCard key={course.id} course={course} isFav={isFavorite(course.id)} onToggleFav={() => toggleFavorite(course.id)} onClick={() => { setSelectedCourse(course); setView("map"); }} cheapestPrice={cheapestTime(course)} filterDay={filters.day} />)}
</div>
      )}
      {view === "favorites" && (
<div style={{ position: "absolute", inset: 0, overflowY: "auto", paddingTop: 120, paddingBottom: 20, background: "#060e06" }}>
          {favoriteCourses.length === 0 ? <EmptyState message="No saved courses yet. Tap ♥ on any course to save it." /> : favoriteCourses.map((course) => <CourseListCard key={course.id} course={course} isFav={true} onToggleFav={() => toggleFavorite(course.id)} onClick={() => { setSelectedCourse(course); setView("map"); }} cheapestPrice={cheapestTime(course)} filterDay={filters.day} />)}
</div>
      )}
      {selectedCourse && <CourseSheet course={selectedCourse} isFavorite={isFavorite(selectedCourse.id)} onToggleFavorite={toggleFavorite} onClose={() => setSelectedCourse(null)} filters={filters} />}
